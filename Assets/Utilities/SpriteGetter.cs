...
public static class FurnitureUtility {
	...
	public static string GetFurnitureSpriteName(int x, int y, string objectType){
		string spriteName = "_";
		Queue<string> queue = new Queue<string>();
		for (int i = 0; i < 8; i++)
		{
			if(angles.ContainsKey(i) == false) continue;
			Tile tile = getTileFromPositionAndVector(x, y, angles[i].Key);
			if(tile != null && tile.Furniture != null && tile.Furniture.Id == objectType){
